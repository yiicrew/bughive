...
class FieldConfigController extends Controller
{
    ...
    public function actionDisable()
    {
        ...
        if ($editResult['status'] == CommonService::$ApiResult['SUCCESS']) {
            $this->redirect(['index', 'product_id' => $_GET['product_id'], 'type' => $_GET['type']]);
        } else {
            throw new CHttpException(400, Yii::t('Common', 'Required URL not found or permission denied.'));
        }
    }
    ...
    public function actionEdit()
    {
        ...
        $this->breadcrumbs = [
            Yii::t('Product', 'Back To Product List') => ['/product/index'],
            $productInfo->name . ' [' . ucfirst($_GET['type']) . Yii::t('Common', 'Manage Fields') . ']' => ['index', 'product_id' => $productId, 'type' => $type],
            $actionName,
        ];
        ...
        $this->render('edit', [
            ...
            'returnUrl' => Yii::app()->createUrl('fieldConfig/index', ['product_id' => $productId, 'type' => $type]),
        ]);
    }
