...
class SearchController extends Controller
{
    ...
    public function actiongetPreNextId()
    {
        ...
        $listStr = CHtml::button(Yii::t('Common', 'Previous') . '(P)',
            ['onclick' => $needToConfirmStr . 'location.href="' .
    ...
    public function actionCheckTemplate()
    {
        ...
        $existedTemplate = UserTemplate::model()->findByAttributes(['title' => $title,
    ...
    public function actionMark()
    {
        ...
        if ('1' == $isAdd) {
            ...
            if ('bug' == $type) {
                $markInfo = MapUserBug::model()->findByAttributes(['info_id' => $infoId, 'test_user_id' => Yii::app()->user->id]);
            } elseif ('case' == $type) {
                $markInfo = MapUserCase::model()->findByAttributes(['info_id' => $infoId, 'test_user_id' => Yii::app()->user->id]);
            } elseif ('result' == $type) {
                $markInfo = MapUserResult::model()->findByAttributes(['info_id' => $infoId, 'test_user_id' => Yii::app()->user->id]);
            }
            if ($markInfo !== null) {
                $markInfo->delete();
            }
    ...
    public function actionDeleteTemplateOrQuery()
    {
        ...
        $type = $_GET['type'];
        if ('template' == $type) {
            $tmpInfo = UserTemplate::model()->findByAttributes(['id' => $id, 'created_by' => Yii::app()->user->id]);
        } elseif ('query' == $type) {
            $tmpInfo = UserQuery::model()->findByAttributes(['id' => $id, 'created_by' => Yii::app()->user->id]);
        }
    ...
    public function actionInfoImport()
    {
        ...
        if ('' != $fileCheckResult) {
            echo json_encode(['msg' => $fileCheckResult]);
        ...
        if (CommonService::getMaxFileSize() < $fileSize) {
            echo json_encode(['msg' => Yii::t('Common', 'Max file size exceeded')]);
        ...
        echo json_encode(['error' => '', 'msg' => $msg]);
