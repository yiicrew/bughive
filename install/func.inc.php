...
function checkServerVar()
{
    ...
    if (!empty($missing)) {
        return t('bugfree', '$_SERVER does not have {vars}.', ['{vars}' => implode(', ', $missing)]);
    }
...
function getDirRights($paths)
{
    ...
    if (is_array($paths)) {
        foreach ($paths as $path) {
            ...
            $dirRights[] = [$path, $readable && $writeable, $readable, $writeable];
        }
    }
...
function getDBConfig($configFile)
{
    ...
    return [$dbhost, $dbname, $port, $dbuser, $dbpwd, $dbprefix];
}
...
{
    mysqli_query('USE ' . $dbname, $con);
    $result = mysqli_query('SHOW TABLES', $con);
    $dbVersion = -1;
    ...
    if ('' != $tableName) {
        ...
        $result1 = mysqli_query($sql, $con);
        $versions = mysqli_fetch_row($result1);
...
function backupDB($con, $dbname, $dbprefix)
{
    ...
    $dumpSql = t('bugfree', 'BugFree backup header {version}', ['{version}' => $version]);
    while ($row = mysqli_fetch_array($result)) {
...
function table2sql($con, $table, $withData = true)
{
    ...
    $createTable = mysqli_query('SHOW CREATE TABLE `' . $table . '`', $con);
    ...
    if ($withData) {
        ...
        $result = mysqli_query('SELECT * FROM `' . $table . '`', $con);
        while ($rows = mysqli_fetch_array($result, MYSQL_ASSOC)) {
            ...
            foreach ($rows as $key => $val) {
                $fields .= $comma . '`' . $key . '`';
                $values .= $comma . '"' . mysqli_real_escape_string($val) . '"';
                $comma = ',';
            }
...
function upgrade($con, $dbname, $dbprefix = '', $dbVersion = null, $step = 1)
{
    ...
    return [$result, $info, $dbVersion, $step];
}
...
function executeSql($con, $sql)
{
    ...
    for ($i = 0; $i < count($ret); $i++) {
        ...
        if (!empty($ret[$i]) && $ret[$i] != "#") {
            $result = $result && mysqli_query($con, $ret[$i]);
            if (!$result) {
                $infos[] = mysqli_error($con);
            }
        }
    }
